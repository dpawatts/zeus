	...
	public class ContentPersister : IPersister
	{
		...
		private void DeleteInboundLinks(ContentItem itemNoMore)
		{
			foreach (LinkProperty detail in _linkFinder.Items().Where(ld => ld.LinkedItem == itemNoMore))
			{
				...
				object test = detail.EnclosingItem.DetailsInternal; // TODO: Investigate why this is necessary, on a PersistentGenericMap
				detail.EnclosingItem.DetailsInternal.Remove(detail);
