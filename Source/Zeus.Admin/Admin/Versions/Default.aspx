<%@ Page Language="C#" MasterPageFile="~/Admin/PreviewFrame.Master" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="Zeus.Admin.Versions.Default" %>
		...
		<asp:GridView ID="gvHistory" runat="server" AutoGenerateColumns="false" DataKeyNames="ID" CssClass="tb"
			...
			<Columns>
				...
				<asp:TemplateField HeaderText="Edit" ItemStyle-Width="50">
					<ItemTemplate>
						<asp:HyperLink runat="server" ID="hlEdit" NavigateUrl='<%# Engine.AdminManager.GetEditExistingItemUrl((Zeus.ContentItem) Container.DataItem) %>'><asp:Image runat="server" ImageUrl='<%# WebResourceUtility.GetUrl(typeof(Zeus.Admin.Versions.Default), "Zeus.Admin.Resources.page_edit.png") %>' AlternateText="Edit" /></asp:HyperLink>
					</ItemTemplate>
				</asp:TemplateField>
				<asp:TemplateField HeaderText="Publish" ItemStyle-Width="50">
					<ItemTemplate>
						<asp:ImageButton runat="server" ID="btnPublish" AlternateText="Publish" CommandName="Publish" CommandArgument='<%# Eval("ID") %>' Visible="<%# !IsPublished(Container.DataItem) %>" ImageUrl='<%# WebResourceUtility.GetUrl(typeof(Zeus.Admin.Versions.Default), "Zeus.Admin.Assets.Images.Icons.book_next.png") %>' />
					</ItemTemplate>
				</asp:TemplateField>
				<asp:TemplateField HeaderText="Delete" ItemStyle-Width="50">
					<ItemTemplate>
						<asp:ImageButton runat="server" ID="btnDelete" AlternateText="Delete" CommandName="Delete" CommandArgument='<%# Eval("ID") %>' Visible="<%# !IsPublished(Container.DataItem) %>" ImageUrl='<%# WebResourceUtility.GetUrl(typeof(Zeus.Admin.Versions.Default), "Zeus.Admin.Resources.page_delete.png") %>' />
					</ItemTemplate>
				</asp:TemplateField>
			</Columns>
		</asp:GridView>
