	...
	public partial class PageCachingUserControl : PluginUserControlBase
	{
		...
		public void ShowDialog(int id)
		{
			...
			btnSave.Listeners.Click.Handler = string.Format(
				"stbStatusBar.showBusy(); Ext.net.DirectMethods.PageCaching.SavePageCachingSettings({0}, Ext.getCmp('{1}').getValue(), Ext.getCmp('{2}').getValue(), {{ url: '/admin/default.aspx', success: function() {{ stbStatusBar.setStatus({{ text: 'Saved page caching settings', iconCls: '', clear: true }}); }} }}); {3}.close();",
				id, chkEnableCache.ClientID, tmeCacheDuration.ClientID, window.ClientID);
