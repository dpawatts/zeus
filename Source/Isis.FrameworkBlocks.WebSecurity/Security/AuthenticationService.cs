...
namespace Isis.Web.Security
{
	public class AuthenticationService : IAuthenticationService
	{
		...
		public AuthenticationService(IWebContext webContext, AuthenticationLocation config)
		{
		...
		public AuthenticationTicket Decrypt(string encryptedTicket)
		{
			if (string.IsNullOrEmpty(encryptedTicket) || encryptedTicket.Length > 0x1000)
				throw new ArgumentException(string.Format(Resources.InvalidArgumentValue, "encryptedTicket"));
...
// ReSharper restore PossibleNullReferenceException
		...
		public void CreateOrUpdateCookieFromTicket(AuthenticationTicket ticket, HttpCookie cookie)
		{
			...
			if (string.IsNullOrEmpty(cookieValue))
				throw new HttpException(Resources.UnableToEncryptCookieTicket);
		...
		public void SetAuthCookie(string userName, bool createPersistentCookie)
		{
			if (!_webContext.Request.IsSecureConnection && _config.RequireSsl)
				throw new HttpException(Resources.ConnectionNotSecureCreatingSecureCookie);
